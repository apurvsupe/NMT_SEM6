...
# %%
class Config:
    INPUT_FILE_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/input/parallel-corpus/Tatoeba.en-mr.en")
    TARGET_FILE_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/input/parallel-corpus/Tatoeba.en-mr.mr")

    OPUS_IN_FILE_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/input/opus-datasets/XLEnt.en-mr.en")
    OPUS_OUT_FILE_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/input/opus-datasets/XLEnt.en-mr.mr")

    LOKMAT_IN_FILE_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/input/lokmat-lifestyle/lokmat-lifestyle_train.en")
    LOKMAT_OUT_FILE_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/input/lokmat-lifestyle/lokmat-lifestyle_train.mr")


    KAGGLE_FILE_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/input/parallel-corpus/data.csv")

    CHECKPOINT_PATH : pathlib.PosixPath = pathlib.Path("/kaggle/working/model_28_03_v2.keras")
    ...
    SPM_INPUT_MODEL_PATH = pathlib.Path("/kaggle/working/en_spm.model")
    SPM_TARGET_MODEL_PATH = pathlib.Path("/kaggle/working/mr_spm.model")
...
class Helper:
    ...
    def load_dataset(input_path: pathlib.PosixPath, target_path: pathlib.PosixPath = None):
            ...
            return (input_lines, target_lines) if target_path else (input_lines, None)
...
def translate_sentence(sentence, encoder, decoder, helper):
    ...
    raw = sentence
    sentence = helper.normalize_text(sentence)
...
def translate_sentence_dummy(sentence, encoder, decoder, helper):
    # Preprocess the input sentence
    sentence = helper.normalize_text(sentence)
    sentence = helper.INPUT_TOKENIZER.encode(sentence)  # FIXED
...
# %%
model1 = tf.keras.models.load_model(r"C:\College\General\Code\sem_6\project\models\en_mr_model_v2.keras",custom_objects={"Encoder" : Encoder, "Decoder": Decoder, "Seq2Seq" : Seq2Seq}, compile=False)

# %%
dummy = translate_sentence_dummy("I hate you!.", model1.encoder, model1.decoder, helper)
model1.compile(optimizer=optimizer, loss="sparse_categorical_crossentropy", metrics=["accuracy"])
model1.load_weights(r"C:\College\General\Code\sem_6\project\models\en_mr_model_weights.weights.h5")

print(dummy)
# print(sentence)
sent = translate_sentence("I hate you!.", model1.encoder, model1.decoder, helper)
print(sent)
